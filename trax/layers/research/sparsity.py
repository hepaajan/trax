...
def LowRankCausalAttention(d_feature, n_heads=1, dropout=0.0,  # pylint: disable=invalid-name
  ...
  Like `CausalAttention`, this layer type represents one pass of multi-head
  self-attention with causal masking rather than padding-based masking. However,
  it uses low-rank approximation of kernel in Dense layer for computing K/Q/V.
...
@assert_shape('bld->bld')
def MultCausalAttention(d_feature, sparsity=1, n_heads=1, dropout=0.0,  # pylint: disable=invalid-name
                        max_inference_length=2048, mode='train'):
  """Returns a layer that maps activations to activations, with causal masking.

  Like `CausalAttention`, this layer type represents one pass of multi-head
  self-attention with causal masking rather than padding-based masking. However,
  instead of a Dense layer for computing K/Q/V it uses multiplicative per-head
  preprocessing followed by a smaller dense layer with weight shared across
  all heads.
  ...
  Args:
    ...
    mode: One of `'train'`, `'eval'`, or `'predict'`.
  """
  # `sparsity` will be usually equal to `n_heads`
  ...
  def ProcessingLayer():  # pylint: disable=invalid-name
    return tl.Serial(
        # Weight used for per-head processing of an embedding.
        tl.Weights(init.RandomNormalInitializer(stddev=0.5),
                   shape=[sparsity, d_feature]),
        # Weight is Dense kernel weight, shared across heads.
        ...
        tl.Fn('AttentionEinsum',
              (lambda d, w, emb: np.einsum('dx,bld,hd->blhx', d, emb, w))),
        MergeLastTwoAxes(),
        # Bias after Dense layer, but specific for each head.
